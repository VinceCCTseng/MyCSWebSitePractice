...
public class sqlAccess
{
    ...
    // 26082016 for update the info of the order
    enum ORDERITEM
    {
        ORDERID     = 1,
        ODATE       = 2,
    ...
    // for "orders"
    public const string ORDERID = "order_ID", ODATE = "order_Date", ONAME = "order_Name", OMEMBER = "memberID", OPHONE = "order_Phone", OADDRESS = "order_Address", OGRAND = "order_Totalcost", OPOSTFEE = "order_Post_Fee", OPOSTDATE = "order_Post_Date", OCOMMENT = "order_Commend", PRODUCTID = "product_ID", PRODUCTQTY = "product_Order_Qty";
    ...
    //IV-1. Insert into order 26082016
    public int InsertAnOrder(string date, string name, string phone, string address, double post,double total)//,
    {
        var sqlinsertorder = "INSERT INTO orders (" + ODATE + ", " + ONAME + ", " + OPHONE + ", " + OADDRESS + ", " + OPOSTFEE + ", " + OGRAND + ") VALUES (@_date, @_name, @_phone, @_address, @_post, @_total);" +
                                "\nSELECT " + ORDERID + " AS LastID FROM orders WHERE "+ORDERID+" = @@Identity;"; //Important
        int orderId =-1;
        try
        {
            using (var cmd = new SqlCommand(sqlinsertorder, conn))
            {
                cmd.Parameters.AddWithValue("@_date", date);
                cmd.Parameters.AddWithValue("@_name", name);
                cmd.Parameters.AddWithValue("@_phone", phone);
                cmd.Parameters.AddWithValue("@_address", address);
                cmd.Parameters.AddWithValue("@_post", post);
        ...
        {
            throw new ApplicationException("[SQL Err] insert order failed!" + ex);            
        }
        return orderId;
    }

    //IV-2. Insert into orderproduct 26082016
    public Boolean InsertAProOfAnOrd(int oid, int pid, int qty)
    {
        var sqlinsertProOrd = "INSERT INTO orderproduct (" + ORDERID + ", " + PRODUCTID + ", " + PRODUCTQTY +") VALUES (@_oid, @_pid, @_qty);";
        try
        {
            using (var cmd = new SqlCommand(sqlinsertProOrd, conn))
            {
        ...
        {
            throw new ApplicationException("[SQL Err] insert product into order failed!" + ex);
        }
        return true;
    }
    //IV-3 update
    //    public Boolean UpdateAnOrder(int customerid, int updateitemid, string itemdetail)
    public Boolean UpdateAnOrder(int customerid, int updateitemid, string itemdetail)
    {
